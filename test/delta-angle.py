...
def delta_analysis(a,b,dist):
    ...
    def delta_increase():
        ...
        if abs(_new_b) >= pi/2:
            _c2 *= -1
        if a not in (-pi,0,pi):
            _new_a = (pi/2 - asin((cos(_d)*radius)/(cos(_new_b)*dist))*sgn_quad(a)*_c)*sgn(a)
        else:
            _new_a = a
        if _c2 < 0:
            _new_a = abs(pi - abs(_new_a)) * (-sgn(_new_a))
        return _new_b, _new_a, _d
    new_a, new_b, _delta = delta_increase()
    return _delta, radius, new_a, new_b

a,b = 0, 0

for _ in range(360):
    a, b = delta_analysis(a, b, 100)